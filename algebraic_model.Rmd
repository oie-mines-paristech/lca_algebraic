...
This notebook presents **lca-algebraic** a small layer above **brightay2**, designed for the definition of **parametric inventories** with super fast computation of LCA impacts, suitable for **monte-carlo** analyis.

**lca-algebraic** provides 3 main functions :
* **helpers** : Provides a set of  **helper functions**, for **compact** & **human readable** definition of activites :  
  * search background (tech and biosphere) activities 
  * create new foreground activites with parametrized amounts
  * parametrize / update existing background activities
* **params** : 
  * Adds definition of parameters and methods for fast computation of LCA
* **stats** :
  * provides functions for statistics analysis of parametric models
  
# Principles 

The main idea of this libray is to move from **procedural definition** of models (slow and prone to errors) to a **declarative / purely functionnal** definition of parametric models (models as static functions). 
...
When computing LCA for foreground models, the library computes **once for all** the impact of **background activities** and compiles a **fast numpy** (vectorial) function for each impact, replacing each background activity by the static value of the impact.
...
The amounts are stored as **float values** or **serialized as string** in the attribute **formula**. 
...
# This is better to cleanup the whole foreground each time, 
...
a = newFloatParam(
    'a', 
    default=0.5, min=0, max=2, # We define a distribution, linear by default
    description="hello world")

b = newFloatParam(
    'b',
    default=0.5, min=0.2, max=4, distrib=DistributionType.TRIANGLE, # Triangular distribution 
    description="foo bar")

share_recycled_aluminium = newFloatParam(
    'share_recycled_aluminium',  
    default=0.6, min=0, max=2, std=0.2, distrib=DistributionType.TRIANGLE, # Normal distrib, with std dev
...
alu2.updateExchanges({
    ...
    "electricity, high voltage*": elec_mix,
...
```
 ...
 Those functions can be called separately or shown as an interactive dashboard
...
```{python}
# Display a matrix of variation of impacts for each parameter
oat_matrix(model, impacts)
```

## Monte-carlo methods & Sobol indices

Here we leverage fast computation of monte-carlo apparoches. Displayed in a unique dashboard.
