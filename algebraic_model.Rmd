...
alu = findTechAct(in_name="aluminium alloy production, AlMg3", loc="RER")
...
activity1 = newActivity(
    "act1", "kg", # Any extra named arg will be added as activity attribute
...
elec_switch = newSwitchAct("elec", elec_switch_param, {
    "us" : us_elec,
    "eu" : eu_elec})
```
...
You can copy and update an existing inventory
Some new helper methods have been added to the class **Activity** for easy update of exchanges

```{python}
alu2 = copyActivity(alu, "Alu2")

# Update exchange by name 
# The special *old_amount* can be used to reference the previous value of the original inventory
...
# Substitute an exchange with a "switch" activity, defaulting to the previous activity
# Here Electricity is replaced by the switch on electricity mix defined above. 
# If elec_switch_param is None at the time of evaluation, the former electricy mix will be used
alu2.substituteWithDefault(
