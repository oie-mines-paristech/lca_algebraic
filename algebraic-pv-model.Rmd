...
```{python}
parametric_oat_interact(impact_model_per_kWp, impacts)
```

```{python}
incer_heatmap_oat(impact_model_per_kWp, impacts)
```

# Statistics : Stochastics

```{python}
incer_heatmap_stochastic(impact_model_per_kWp, impacts)
```
...
plt.bar(np.arange(len(param_names)), res["ST"], 0.8)
plt.bar(np.arange(len(param_names)), res["S1"], 0.8)
plt.xticks(np.arange(len(param_names)), param_names, rotation=90)
```

```{python}
incer_violin_stochastic(impact_model_per_kWh, impacts)
```
