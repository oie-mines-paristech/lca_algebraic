...
```{python}
# Params 
silver_amount = newFloatParam("silver_amount", 9.6)
...
```{python}
# Params 
recycling_rate_alu = newFloatParam("recycling_rate_alu", 0.96) 
recycling_rate_cu = newFloatParam("recycling_rate_cu", 0.75)
recycling_rate_glass = newFloatParam("recycling_rate_glass", 0.9),
electricity_recycling = newFloatParam("electricity_recycling", 50)
heat_recycling = newFloatParam("heat_recycling", 76)
glass_thickness = newFloatParam("glass_thickness", 4) 
...
module.addExchanges({
    scrapAlu : amount_alu,
    scrapCopper : amount_copper,
    scrapGlass : amount_glass,
...
module.updateExchanges({
    'aluminium alloy, AlMg3' : dict(input=alu_adjusted),
    'solar glass, low-iron' : glass_thickness * 2.5 * 1,
    'tempering, flat glass' : glass_thickness * 2.5 * 1,
    'photovoltaic cell, multi-Si wafer' : dict(input=wafer_adjusted),
})

module.substituteWithDefault('electricity, medium voltage', elec)


```

```{python}
printAct(module_init, module, 
    sic_recycled_share=0,
    teg_recycled_share = 0,
    diamond_wiring = 1, 
    glass_thickness = 4, 
