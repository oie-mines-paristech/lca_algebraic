...
elec_glo = findTechAct(ELEC_GROUP_NAME, "GLO")
```

```{python}
# Virtual activity corresponding to switch upon elec mixes
elec = newSwitchAct(USER_DB, "elec", elec_switch, {
    "us" : elec_us,
    "eu" : elec_eu,
    "fr" : elec_fr,
    "cn" : elec_cn,
...
```{python}
multiLCA([elec_us, elec_eu, elec_glo, elec_cn, elec_fr], impacts)
```
...
```{python}
alu_adjusted = copyActivity(USER_DB, alu, "Alu adjusted")
...
ground_coverage_ratio = newFloatParam(
    ...
    label_fr="ratio de couverture au sol",
    unit="fraction")
...
```{python}
mounting_system_modified = copyActivity(USER_DB, ground_system, "mounting system adjusted")
...
inverter_per_kg = newInterpolatedAct(USER_DB,
...
```{python}
printAct(inverter_per_kg, P=3000, recycling_rate=0.3)
```
...
metal_paste_copper = copyActivity(USER_DB, metal_paste, 'metal paste, copper')
...
silicon_adjusted = copyActivity(USER_DB, silicon, "silicon production adjusted")
...
silicon_casting_adjusted = copyActivity(USER_DB, silicon_casting, 'silicon casting adjusted')
...
silicon_carbide_recycled = copyActivity(USER_DB, silicon_carbide, 'silicon carbid adjusted', withExchanges=False)
...
teg_recycled = copyActivity(USER_DB, teg, "TEG recycled", withExchanges=False)
...
```{python}
tungstene_carbide = copyActivity(USER_DB, silicon_carbide, "tungstene carbide", withExchanges=False)
...
diamond_powder = copyActivity(USER_DB, diamond_powder_ori, "Diamond powder",  withExchanges=False)
...
sintered_diamond = copyActivity(USER_DB, sintered_diamond_ori, "sintered diamond bead, for quarrying", withExchanges=False)
...
sintered_diamond_wire = copyActivity(USER_DB, act_ori, "sintered diamond wire, steel", withExchanges=False)
...
kerf_loss = newFloatParam(
    "kerf_loss", 0.44, min=0.40, max=0.50,
    group="manufacturing", 
    label="kerf loss", 
...
kerf_loss_adapted = kerf_loss - 0.15 * diamond_wiring
sili_casting_amount = wafer_thickness * 1e-6 * 2328 / (1 - kerf_loss_adapted)
non_diamond_wiring = 1 - diamond_wiring


# Copy Wafer
wafer = findTechAct("multi-Si wafer production", "RER")
wafer_adjusted = copyActivity(USER_DB, wafer, "Wafer adjusted")
...
cell_adjusted = copyActivity(USER_DB, cell_init, 'photovoltaic cell production, multi-Si wafer - adjusted')
...
panel_adjusted = copyActivity(USER_DB, panel_init, 'photovoltaic panel production, multi-Si wafer - adjusted')
...
```{python}
pvInstall = newActivity(USER_DB, "Full pv installation", "unit")
...
impact_model_per_kWp = newActivity(USER_DB, "impact model per kWp installed", unit="unit per kWh", exchanges={
    pvInstall: 1 / P
})
```
...
impact_model_per_kWh = newActivity(USER_DB, "impact model per kWh produced over lifetime", unit="unit per kWh", exchanges={
    pvInstall: 1 / productible_life * P
})
