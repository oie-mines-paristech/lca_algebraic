...
# del bw.databases['ecoinvent 3.4 cut off']
...
def print_data(act):
    ...
    for exc in act.exchanges():
        name = exc['name']
        input = dbs[exc.input.key[0]].get(exc.input.key[1])
        if exc.input.key[0] == 'ecoinvent 3.4 cut off' :
            author = input['author'] if 'author' in input else input['authors']
            if "Romain" in author or 'Isabella' in author:
                name += '{incer}'
        if exc['name'] in df.columns:
            df[name].loc['amount'] += exc.amount
        else:
            df[name] = [exc.amount, exc.unit, input['name']]
...
```{python}
def lca(*acts) :
    bw.calculation_setups['foo'] = {'inv': [{act : 1} for act in acts], 'ia': set_of_methods}
    myMultiLCA = bw.MultiLCA('foo')
...
def generate_silicon_dataset(Silicon_electricity_intensity = 30+10, elec_dataset = None):
    ...
    for exc in Silicon_adjusted.exchanges():
        if exc['name'] == 'electricity, high voltage':
            exc['input'] = elec_dataset.key if elec_dataset != None else Silicon.getExchange('electricity, high voltage#RER')['input']
            exc.save()
...
```{python}
casting_act_ori = [act for act in eidb if 'silicon production, multi-Si, casted' in act['name'] and 'RoW' in act['location']][0]

bw.calculation_setups['B_SiC'] = {'inv': [{casting_act_ori:1}, {casting_act:1}], 'ia': list_methods}
...
def generate_diamond_powder():
    ...
    if len([act for act in eidb if 'diamond powder, production' in act['name']]) == 0 :
        ...
        #input
        act = [act for act in eidb if 'market for electricity, medium voltage' == act['name'] and 'RoW' in act['location']][0]
...
```{python code_folding=c()}
def generate_sintered_diamond_bead(out_amount=0.004):
    ...
    if len([act for act in eidb if 'sintered diamond bead, for quarrying' == act['name']]) == 0 :
        ...
        new_exc = sintered_diamond_bead.new_exchange(
            input = sintered_diamond_bead.key, 
            amount = out_amount, unit = 'kilogram', type = 'production', name=sintered_diamond_bead['name'])
        new_exc.save()
        sintered_diamond_bead.save()
...
sintered_diamond_bead = generate_sintered_diamond_bead(0.004)
...
bw.calculation_setups['B_sintered_diamond_b1'] = {'inv': [{sintered_diamond_bead:1}], 'ia': list_methods}
myMultiLCA = bw.MultiLCA('B_sintered_diamond_b1')
df_impact = pd.DataFrame(columns = list_methods, data = myMultiLCA.results)
df_impact
```
...
def generate_wafer_dataset(SiC_recycled_share=0.694, TEG_recycled_share = 0.899, 
    ...
    for exc in wafer_adjusted.exchanges():
        if exc['name'] == 'triethylene glycol' :
            exc['amount'] = 0 if DW else 2.16548 * (1 - TEG_recycled_share)
            exc.save()

        if exc['name'] == 'silicon carbide' :
            exc['amount'] = 0 if DW else 2.02988 * (1 - SiC_recycled_share)
            exc.save()

        if exc['name'] == 'tap water' :
            #exc['amount'] = exc['amount'] + 98.94
            exc['amount'] = 0.006 + (98.94 if DW else 0)
            exc.save()

        if exc['name'] == 'sintered diamond wire, steel' :
            exc['amount'] = 0.0006075 if DW else 0 
            exc.save()

        if exc['name'] == 'silicon carbide, recycled production':
            exc['amount'] = 0 if DW else 2.02988 * SiC_recycled_share
            exc.save()

        if exc['name'] == 'triethylene glycol, recycled production':
            exc['amount'] = 0 if DW else 2.16548 * TEG_recycled_share
            print("been there : ", DW, TEG_recycled_share)
            exc.save()
...
```{python}
wafer_adjusted = [act for act in eidb if 'multi-Si wafer production adjusted' == act['name'] and 'RER' in act['location']][0]
...
bw.calculation_setups['B_wafer_original'] = {'inv': [
    {wafer_ori:1} , 
    {wafer_adjusted:1},
    {casting_act:1}, 
    {SiC_recycled:1}, 
...
panel_adjusted = generate_panel_dataset(SiC_recycled_share=0,
                                        ...
                                        wafer_thickness = 240, 
                                        silver_amount = 11.2,
                                        Silicon_electricity_intensity = 40, 
                                        print_steps = True, 
...
```{python}
lca(panel_ori, panel_adjusted, alu_adjusted, wafer_adjusted)
```
...
```{python}
print_data(panel_adjusted)
```

```{python}
print_data(panel_ori)
```
...
list_methods = set_of_methods
bw.calculation_setups['B_pv_panel'] = {'inv': [{panel_ori:1}, {panel_adjusted:1}, {alu_adjusted:1}, {wafer_adjusted:1}], 'ia': list_methods}
...
```{python}
list_methods = set_of_methods
bw.calculation_setups['B_system'] = {'inv': [
    {system_adjusted:1},
    {panel_adjusted:1},
    {wafer_adjusted:1},
], 'ia': list_methods}
